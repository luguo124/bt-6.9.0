...
bt_domain=https://api.moecat.cn/6.9.0_v2
MAIN_RETURN=${Red_font}[宝塔面板v6.9.0_v2开心脚本]${Font_suffix}
...
# 创建下载我破解好的文件啦~
get(){
    mkdir /root/bt_6.9.0_v2
	cd /root/bt_6.9.0_v2
	wget -N --no-check-certificate ${bt_domain}/bt_6.9.0_v2.zip
	unzip -o bt_6.9.0_v2.zip > /dev/null	
}

# 进行cp所需要的文件啦~
copy(){
    cp -pf /root/bt_6.9.0_v2/__init__.py ${PANEL_DIR}/BTPanel/__init__.py
	cp -pf /root/bt_6.9.0_v2/panelPlugin.py ${PANEL_DIR}/class/panelPlugin.py
	cp -pf /root/bt_6.9.0_v2/ajax.py ${PANEL_DIR}/class/ajax.py
	cp -pf /root/bt_6.9.0_v2/public.py ${PANEL_DIR}/class/public.py
    cp -pf /root/bt_6.9.0_v2/soft.js ${PANEL_DIR}/BTPanel/static/js/soft.js
	cp -pf /root/bt_6.9.0_v2/index.html ${PANEL_DIR}/BTPanel/templates/default/index.html	
}
