...
if [ "${vp}" == "free" ]; then
	...
	updateApi=https://loli.team/bt5/Api/updateLinuxBeta
	vp=""
	Ver="内测版"
fi
...
if [ ! -f $public_file ];then
	wget -O $public_file https://loli.team/bt5/install/public.sh -T 5;
fi
. $public_file
...
if [ "$version" = '' ];then
	if [ "${updateApi}" == "" ];then
		updateApi=https://loli.team/bt5/Api/updateLinux
	fi
...
wget -T 5 -O panel.zip https://loli.team/bt5/install/update/LinuxPanel-${version}${vp}.zip
if [ ! -f "panel.zip" ];then
...
if [ "${check_bt}" = "" ];then
	rm -f /etc/init.d/bt
	wget -O /etc/init.d/bt https://loli.team/bt5/install/src/bt.init -T 10
