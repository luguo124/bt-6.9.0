...
if [ -f "/usr/bin/apt-get" ];then
	...
	if [ "$isDebian" != "" ];then
		wget -O install.sh https://raw.githubusercontent.com/luguo124/bt-6.9.0/master/install-ubuntu.sh && bash install.sh
		exit;
	else
		wget -O install.sh https://raw.githubusercontent.com/luguo124/bt-6.9.0/master/install-ubuntu.sh && sudo bash install.sh
		exit;
	fi
fi
...
"
get_node_url(){
	nodes=(https://loli.team https://loli.team https://loli.team https://loli.team);
	i=1;
	if [ ! -f /bin/curl ];then
...
if [ "$kernelStatus" = "" ]; then
	if [ "$huaweiLogin" != "" ] && [ "$huaweiSys" != "" ]; then
		wget $download_Url/bt5/src/kernel-headers-3.10.0-514.el7.x86_64.rpm  # ok
...
Install_setuptools()
{
	if [ ! -f "/usr/bin/easy_install" ];then
		wget -O setuptools-33.1.1.zip $download_Url/bt5/install/src/setuptools-33.1.1.zip -T 10
...
Install_pip()
{
	...
	if [ "$ispip" == "" ];then
		...
		wget -O pip-9.0.1.tar.gz $download_Url/bt5/install/src/pip-9.0.1.tar.gz -T 10
...
Install_Pillow()
{
	...
	if [ "$isSetup" = "" ];then
		...
		wget -O Pillow-3.2.0.zip $download_Url/bt5/install/src/Pillow-3.2.0.zip -T 10
...
Install_psutil()
{
	...
	if [ "$isSetup" = "" ];then
		wget -O psutil-5.2.2.tar.gz $download_Url/bt5/install/src/psutil-5.2.2.tar.gz -T 10
...
Install_mysqldb()
{
	...
	if [ "$isSetup" = "" ];then
		wget -O MySQL-python-1.2.5.zip $download_Url/bt5/install/src/MySQL-python-1.2.5.zip -T 10
...
Install_chardet()
{
	...
	if [ "$isSetup" = "" ];then
		wget -O chardet-2.3.0.tar.gz $download_Url/bt5/install/src/chardet-2.3.0.tar.gz -T 10
...
Install_webpy()
{
	...
	if [ "$isSetup" = "" ];then
		wget -O web.py-0.38.tar.gz $download_Url/bt5/install/src/web.py-0.38.tar.gz -T 10
...
wget -O $setup_path/server/panel/certbot-auto $download_Url/bt5/install/certbot-auto.init -T 5
...
wget -O panel.zip $download_Url/bt5/install/src/panel.zip -T 10
wget -O /etc/init.d/bt $download_Url/bt5/install/src/bt.init -T 10
...
wget -O acme_install.sh $download_Url/bt5/install/acme_install.sh
